        ...
        # Optional LLM review
              ...
              if sig.action in ("buy", "sell"):
                            ...
                            if dec is not None:
                                              ...
                                              if not dec.approve:
                                                                    decisions.append(
                                                                                              Decision(symbol, "hold", 0.0, f"LLM vetoed: {sig.reason}{llm_note}", sig.score, llm_ok)
                                                                    )
                                                                    continue
                                      ...
                                      if sig.action == "buy":
                                                    ...
                                                    if remaining_budget <= 10 or cash < 10:
                                                                      decisions.append(
                                                                                            Decision(symbol, "hold", 0.0, f"at cap or no cash ({sig.reason}){llm_note}", sig.score, llm_ok)
                                                                      )
                                                                      continue
